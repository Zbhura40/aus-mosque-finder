...
async function fetchPerthReviews() {
  console.log('Fetching Reviews for Perth (WA) Mosques');
  console.log('=======================================\n');
  ...
  console.log(`Found ${mosques.length} Perth mosques to process\n`);
  ...
  for (let i = 0; i < mosques.length; i++) {
    ...
    try {
      ...
      const { error: updateError } = await supabase.from('mosques_cache').update({ reviews }).eq('id', mosque.id);
